...
- Every class in `application/command/` implements `Command<Input, Output>`.
- Every class in `application/query/` implements `Query<Input, Output>`.
- Import these contracts from `@/systems/cqrs`; keep `Input` and `Output` types colocated with their command or query.
